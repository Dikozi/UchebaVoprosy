...
| Файл | Для чего |
|---|---|
| `Режим · 2026.xlsx` | **Основная сборка** под Google Таблицы и телефон (светлая) |
| `Режим · демо.xlsx` | Три заполненных месяца: для рилсов и скриншотов |
| `Режим · 2026 (тёмный, для компьютера).xlsx` | Тёмный вариант: только для браузера на компьютере |
| `Режим · 2026 · Excel.xlsx` | Бонус для компьютера: нативные флажки Excel 365, полосы вместо SPARKLINE |

**Почему основная тема светлая.** Приложение Google Таблиц в тёмном режиме инвертирует
заливки ячеек: тёмный документ становится светлым, белый текст чёрным. Проверено на телефоне —
тёмная сборка там разваливается. Светлая ведёт себя предсказуемо в обоих режимах приложения.

![Месяц](preview/месяц.png)

## Как выпустить продукт

1. Загрузить `Режим · 2026.xlsx` на Google Диск и открыть как Таблицу (импорт .xlsx).
...
* **`Янв` … `Дек`** — шапка 144 px (месяц и прогресс, строка дня, значки, имена, серии),
  сетка «дни × привычки» 52 px на строку, колонка даты 46 px, плитки привычек 43 px, за сгибом сон, силы, настроение и заметка,
